(ns fibreopticmfg.governor
  "Fibre Optic Cable Plant Operations Governor -- the independent
  compliance layer that earns the FibreOpticCableAdvisor the right to
  ...
  draft-schedule) drawing/coating/cabling-line equipment, whether a
  proposal secretly tries to self-issue a fibre-optic-cable compliance-
  ...
  coordination proposal and becomes direct drawing/coating/cabling-
  ...
  *reject* a proposal and fall back to HOLD.
  ...
  `:fibre-optic-cable-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:draw-tower/actuate`
                                       or `:cabling-line/run`) is the
                                       'direct drawing/coating/cabling-
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       certification mark (e.g.
                                       Telcordia GR-20, ITU-T G.65x,
                                       UL/NFPA fire-rating compliance)
   ...
   10. Invalid product-type        -- for `:log-production-batch`, if
                                       ...
                                       closed known set
                                       (`fibreopticmfg.registry/product-
                                       type-valid?`), the batch record
                                       is rejected rather than let a
                                       fabricated product type through.
   11. Invalid attenuation-db-km   -- for `:log-production-batch`, if
                                       the patch declares an
                                       `:attenuation-db-km` that is not
                                       a physically plausible OTDR/
                                       cutback test reading
                                       (`fibreopticmfg.registry/
                                       attenuation-db-km-valid?`), the
   ...
   12. Invalid defect-rate         -- for `:log-production-batch`, if
                                       ...
                                       reading (`fibreopticmfg.registry/
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [fibreopticmfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct drawing/
  coating/cabling-line-equipment-control effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct drawing/coating/cabling-line-equipment control,
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は延伸・被覆・ケーブル化設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate drawing/coating/cabling-line equipment -- this
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "延伸・被覆・ケーブル化設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue a fibre-optic-cable compliance-certification mark (e.g.
  Telcordia GR-20, ITU-T G.65x, UL/NFPA fire-rating compliance) -- an
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "光ファイバケーブル認証(Telcordia GR-20/ITU-T G.65x/UL・NFPA耐火等)の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
      ...
      (when (and b (registry/shipment-quantity-exceeded? b units))
        [{:rule :shipment-quantity-exceeded
          :detail (str batch-id " の記録済み生産数量(" (:quantity-units b)
                       "リール)を、既存出荷実績(" (:shipped-units b 0.0)
                       "リール)+今回申請(" units "リール)が超過")}]))))
...
(defn- invalid-attenuation-db-km-violations
  "For `:log-production-batch`, if the patch declares an
  `:attenuation-db-km` that is not a physically plausible OTDR/cutback
  test reading, reject rather than let fabricated/sensor-error data
  through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [db-km (:attenuation-db-km (:value proposal))]
      (when (and (some? db-km) (not (registry/attenuation-db-km-valid? db-km)))
        [{:rule :invalid-attenuation-db-km
          :detail (str db-km "dB/km は物理的に妥当な減衰試験値の範囲外")}]))))
...
(defn check
  "Censors a FibreOpticCableAdvisor proposal against the governor
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-attenuation-db-km-violations request proposal)
