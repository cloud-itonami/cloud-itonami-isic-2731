(ns fibreopticmfg.sim
  ...
  production quantity, a proposal that tries to ACTUATE drawing/
  coating/cabling-line equipment directly (permanently blocked, no
  override), a proposal that tries to self-issue a fibre-optic-cable
  ...
  an implausible attenuation-db-km reading, and a production-batch
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on draw-tower-001 (verified, registered, fibre-draw tower -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on draw-tower-001 (always escalates -- approve) ==")
    ...
    (println (exec-op actor "t6"
                       {:op :actuate-cabling-line :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on cabling-line-002 (UNVERIFIED/unregistered cabling line -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on draw-tower-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with an implausible attenuation-db-km -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:attenuation-db-km 999999999.0}}
                       coordinator))
    ...
    (println "== log-production-batch batch-001 attempting to self-issue a fibre-optic-cable compliance certification -> HARD hold, PERMANENT ==")
