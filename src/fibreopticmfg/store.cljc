(ns fibreopticmfg.store
  "SSoT for the ISIC 2731 (fibre optic cables) plant-operations
  ...
  Scope note: like its siblings (`cloud-itonami-isic-2733`'s own
  `wiringdevmfg.store`, `cloud-itonami-isic-2790`'s own
  `otherelecmfg.store`), this build ships a single `MemStore` backend
  ...
  is self-contained (no external fibre-optic-cable-manufacturing
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/attenuation-db-km/quantity/
                             ...
                             cumulative-shipped ground truth (finished
                             cable-reel units).
    - `equipment`         -- a drawing/coating/cabling-line unit's own
    ...
    - `maintenance`       -- a scheduled drawing/coating/cabling-line
                             maintenance-window DRAFT against a piece
                             of equipment (`fibreopticmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound fibre-optic-cable-reel
                             shipment DRAFT (`fibreopticmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (fibreopticmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (fibreopticmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :loose-tube
                :spool-length-km 24.0 :attenuation-db-km 0.35
                :quantity-units 500.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :ribbon
                :spool-length-km 12.0 :attenuation-db-km 0.22
                :quantity-units 200.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 180.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :tight-buffered
                :spool-length-km 2.0 :attenuation-db-km 3.1
...
(defn- sample-equipment []
  {"draw-tower-001" {:id "draw-tower-001" :kind :fibre-draw-tower
                      :verified? true :registered? true
                      :last-maintenance-date "2026-05-01"}
   "cabling-line-002" {:id "cabling-line-002" :kind :cabling-line
                        :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `fibreopticmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `fibreopticmfg.registry` and
...
(defn sample-data!
  ...
  fibre-draw-tower unit (schedulable for maintenance), one UNVERIFIED/
  unregistered cabling-line unit (blocks any maintenance scheduling
