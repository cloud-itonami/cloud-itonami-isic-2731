(ns fibreopticmfg.operation
  "FibreOpticCableOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (FibreOpticCableAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Fibre Optic Cable Plant Operations Governor (:govern) and the
  ...
  (:require [langgraph.graph :as g]
            ...
            [fibreopticmfg.advisor :as advisor]
...
(defn build
  "Compiles a FibreOpticCableOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `fibreopticmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FibreOpticCableAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Fibre Optic Cable Plant Operations Governor -- independent censor.
      (g/add-node :govern
