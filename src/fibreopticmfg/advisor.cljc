(ns fibreopticmfg.advisor
  "FibreOpticCableAdvisor -- the *contained intelligence node* for the
  ISIC 2731 (fibre optic cables: loose-tube, ribbon, tight-buffered,
  ADSS) plant-operations coordination actor.

  It normalizes production-batch patches (product-type/attenuation-
  db-km/quantity/defect-rate), drafts a drawing/coating/cabling-line-
  ...
  fibre-optic-cable-reel shipment coordination proposal against a
  ...
  a committed record and NEVER a real drawing/coating/cabling-line-
  equipment actuation, freight dispatch, or fibre-optic-cable
  compliance certification -- see README `What this actor does NOT
  do`. Every output is censored downstream by `fibreopticmfg.governor`
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `fibreopticmfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct drawing/
                                 ; coating/cabling-line-equipment-control
                                 ; effect
  ...
  `fibreopticmfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  attenuation-db-km, quantity, defect-rate or verification status.
...
(defn- schedule-maintenance
  "Draft a drawing/coating/cabling-line-equipment maintenance-window
  scheduling proposal against a piece of equipment. The advisor
  reports what it can see (equipment verified?/registered?) in its
  rationale, but `fibreopticmfg.governor` NEVER trusts this report --
...
(defn- flag-safety-concern
  "Draft a fibre-breakage-hazard/coating-oven-thermal-hazard/
  ...
  on an administrative technicality). See `fibreopticmfg.phase`: no
  phase ever adds this op to a phase's `:auto` set;
  `fibreopticmfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound fibre-optic-cable-reel shipment coordination
  ...
  `fibreopticmfg.governor` NEVER trusts it: it independently
...
(def ^:private system-prompt
  (str "あなたは光ファイバケーブル(ルースチューブ・リボン・タイトバッファ・"
       "ADSS)製造工場プラント運用コーディネーターの助言者です。"
       ...
       "延伸・被覆・ケーブル化等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "光ファイバケーブル認証(Telcordia GR-20/ITU-T G.65x等)を自己発行する提案を"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `fibreopticmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :fibreopticmfg-advisor-proposal
   :op         (:op request)
