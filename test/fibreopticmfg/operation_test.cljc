(ns fibreopticmfg.operation-test
  "Smoke tests for the compiled FibreOpticCableOperationActor graph
  itself (build + one happy path per op). The governor's full rule
  contract (HARD holds, escalation, phase gating) is exercised in
  `fibreopticmfg.governor-contract-test`; the Store contract in
  `fibreopticmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "FibreOpticCableOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "draw-tower-001" :severity :moderate :description "coating temperature anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
