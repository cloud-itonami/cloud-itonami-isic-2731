(ns fibreopticmfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [fibreopticmfg.registry :as r]))
...
(deftest known-product-types-are-valid
  (doseq [pt [:loose-tube :ribbon :tight-buffered :adss]]
...
;; ----------------------------- attenuation-db-km-valid? -----------------------------

(deftest typical-attenuation-db-km-is-valid
  (is (r/attenuation-db-km-valid? 0.0))
  (is (r/attenuation-db-km-valid? 0.2))
  (is (r/attenuation-db-km-valid? 3.5))
  (is (r/attenuation-db-km-valid? 50.0)))

(deftest negative-attenuation-db-km-is-invalid
  (is (not (r/attenuation-db-km-valid? -1.0))))

(deftest excessive-attenuation-db-km-is-invalid
  (is (not (r/attenuation-db-km-valid? 999999999.0)))
  (is (not (r/attenuation-db-km-valid? 50.01))))

(deftest non-numeric-or-missing-attenuation-db-km-is-invalid
  (is (not (r/attenuation-db-km-valid? nil)))
  (is (not (r/attenuation-db-km-valid? "0.35"))))
...
(deftest history-is-append-only
  (let [c1 (r/register-maintenance "mnt-1" "draw-tower-001" 0)
        hist (r/append [] c1)
        c2 (r/register-maintenance "mnt-2" "draw-tower-001" 1)
