(ns fibreopticmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control drawing/coating/cabling-line
  equipment directly... does NOT authorize or execute
  maintenance/equipment operations... does NOT self-issue a
  fibre-optic-cable compliance-certification mark') implemented
  faithfully. The single invariant under test:

    FibreOpticCableAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Fibre Optic Cable Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :loose-tube}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :loose-tube (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t2"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                     :value {:equipment-id "draw-tower-001" :maintenance-type :coating-die-inspection
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-cabling-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the fibre-optic-cable compliance-certification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-attenuation-db-km-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:attenuation-db-km 999999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-attenuation-db-km} (-> (store/ledger db) last :basis)))
    (is (not= 999999999.0 (:attenuation-db-km (store/batch db "batch-001"))) "fabricated attenuation-db-km never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "draw-tower-001" :severity :moderate
                                            :description "attenuation anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "draw-tower-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
